...
namespace GLTFast
{
    ...
    class VertexBufferConfigInterleaved<VType> :
        ...
        where VType : struct
    {
        ...
        public override unsafe JobHandle? ScheduleVertexJobs(
            ...
            //     vData = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<VType>(input, posInput.count, Allocator.None);
            // }

            offset = posInput.startOffset;
            
            var x= NativeArrayUnsafeUtility.GetAtomicSafetyHandle(posInput.nativeBuffer);
            var input = ((byte*)NativeArrayUnsafeUtility.GetUnsafePtr(posInput.nativeBuffer))+offset;
            vData = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<VType>(input, posInput.count, Allocator.None);
            NativeArrayUnsafeUtility.SetAtomicSafetyHandle(ref vData,x);

            // vData = posInput.nativeBuffer.Reinterpret<VType>(1);
            // offset = posInput.startOffset / 32;
            // vCount = posInput.count;
        ...
        public override void ApplyOnMesh(UnityEngine.Mesh msh, MeshUpdateFlags flags = MeshUpdateFlags.Default) {
            ...
            int stream = 0;
            msh.SetVertexBufferData(vData,0,0,vCount,stream,flags);
            stream++;
            Profiler.EndSample();
